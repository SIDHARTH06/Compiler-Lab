...
VARLIST: VARLIST ',' IDDECL {$$=joinnode($1,$3);}
	 | IDDECL {$$ = $1;}
	 ;
IDDECL: ID {$$=createsymbolnode(gst,$<name>1,INVALIDTYPE,1,1);}
			 | ID '[' NUM ']' {$$=createsymbolnode(gst,$<name>1,INVALIDTYPE,$<number>3,$<number>3);}
			 | ID '[' NUM ']' '[' NUM ']'	{
...
INPUTSTMNT : READ '(' IDENTIFIER ')' ENDOFLINE		{$$ = createTree(-1,INVALIDTYPE,NULL,READNODE,$3,NULL);}
	 ;

OUTPUTSTMNT : WRITE '(' expr ')' ENDOFLINE	{$$ = createTree(-1,INVALIDTYPE,NULL,WRITENODE,$3,NULL);}
	 ;

ASSIGNSTMNT : IDENTIFIER ASSIGN expr ENDOFLINE		{$$ = createTree(-1,INVALIDTYPE,NULL,ASSIGNNODE,$1,$3);}
	 ;
...
IDENTIFIER: ID {struct tnode* temp =createTree(-1,INVALIDTYPE,$<name>1,VARNODE,NULL,NULL);
	 ...
	 ;
	 | ID '[' expr ']' '[' expr ']'  { struct tnode* temp = createTree(-1, INVALIDTYPE, $<name>1,VARNODE,$3,$6);
                                            findtype(gst,temp);
